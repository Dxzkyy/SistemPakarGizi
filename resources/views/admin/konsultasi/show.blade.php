...
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.konsultasi.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-blue-600 transition">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="space-y-5">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="flex items-center gap-4 mb-4">
                <div class="w-14 h-14 bg-blue-100 rounded-2xl flex items-center justify-center">
                    <span class="text-blue-600 text-xl font-bold">{{ strtoupper(substr($konsultasi->user->name, 0, 1)) }}</span>
                </div>
                <div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
                ...
                @if($konsultasi->nilai_bayes)
                    <p class="text-sm text-blue-600 mt-1">Nilai Bayes: <span class="font-bold">{{ $konsultasi->nilai_bayes }}%</span></p>
                @endif
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-green-200 p-6">
            ...
            <div class="bg-green-50 border border-green-200 rounded-xl p-4 mb-4">
                ...
                <p class="text-lg font-bold text-green-800">{{ $konsultasiPakar->hipotesis->nama ?? '-' }}</p>
                @if($konsultasiPakar->nilai_bayes)
                    <p class="text-sm text-green-600 mt-1">Nilai Bayes: <span class="font-bold">{{ $konsultasiPakar->nilai_bayes }}%</span></p>
                @endif
            </div>
            @if($konsultasiPakar->catatan_pakar)
            <div>
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Catatan Pakar</p>
                <p class="text-sm text-gray-700 bg-gray-50 rounded-xl p-3">{{ $konsultasiPakar->catatan_pakar }}</p>
            </div>
            @endif
        </div>
        @endif
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <p class="text-xs text-gray-400 mb-5">Pilih gejala berdasarkan pemeriksaan langsung, kemudian simpan validasi</p>
        ...
        <form action="{{ route('admin.konsultasi.validasi', $konsultasi->id) }}" method="POST">
                ...
                <div class="space-y-2 max-h-[400px] overflow-y-auto pr-1">
                    ...
                    <label class="flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition
                        ...
                        <input type="checkbox" name="gejala[]" value="{{ $gejala->id }}"
                            {{ $sudahDipilihUser ? 'checked' : '' }}
                            class="w-4 h-4 text-blue-600 rounded">
                        <div class="flex-1">
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-bold text-blue-600">{{ $gejala->kode }}</span>
                                @if(!$gejala->tampil_ke_user)
                                    <span class="text-xs bg-purple-100 text-purple-600 px-1.5 py-0.5 rounded">Pakar</span>
                                @endif
                                @if($sudahDipilihUser)
                                    <span class="text-xs bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded">User</span>
                                @endif
                            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="catatan_pakar" rows="3"
                    placeholder="Tambahkan catatan atau saran khusus untuk ibu hamil ini..."
