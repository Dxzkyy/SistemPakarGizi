            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-4">
                    <span class="text-xs font-medium px-2.5 py-1 rounded-full
                        {{ $item->tipe === 'self' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700' }}">
                ...
                <td class="px-6 py-4">
                    @if($item->nilai_bayes)
                        <span class="font-semibold text-gray-800">{{ $item->nilai_bayes }}%</span>
                    @else
                ...
                <td class="px-6 py-4 text-center">
                    <a href="{{ route('admin.konsultasi.show', $item->id) }}"
                       class="text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg text-xs font-medium transition">
                        {{ $item->tipe === 'self' && $item->status === 'pending' ? 'Validasi' : 'Detail' }}
                    </a>
                </td>
            </tr>
            @empty
