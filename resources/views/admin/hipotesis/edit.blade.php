...
<form action="{{ route('admin.hipotesis.update', $hipotesis->id) }}" method="POST">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="mb-5">
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Hipotesis <span class="text-red-500">*</span></label>
            ...
            <div class="mb-5">
                ...
                <textarea name="deskripsi" rows="3"
                    class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('deskripsi', $hipotesis->deskripsi) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Rekomendasi Gizi</label>
                <textarea name="rekomendasi" rows="4"
                    class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('rekomendasi', $hipotesis->rekomendasi) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-3 max-h-[480px] overflow-y-auto pr-1">
                ...
                @endphp
                <div class="flex items-center gap-3 p-3 border rounded-xl transition {{ $checked ? 'border-blue-400 bg-blue-50' : 'border-gray-200 hover:border-blue-300' }}" id="row-{{ $gejala->id }}">
