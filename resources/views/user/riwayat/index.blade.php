                ...
                @foreach ($konsultasi as $index => $item)
                    <div class="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-gray-50 transition">
                        ...
                        <div class="col-span-3">
                            <p class="text-sm font-medium text-gray-800">{{ $item->created_at->translatedFormat('d F Y') }}</p>
                            <p class="text-xs text-gray-400 mt-0.5">{{ $item->created_at->format('H:i') }}</p>
                        </div>
                        <div class="col-span-3">
                            @if($item->status === 'selesai' && $item->hipotesisPakar)
                                <p class="text-sm font-medium text-gray-800">{{ $item->hipotesisPakar->nama ?? '—' }}</p>
                            @else
                                <p class="text-sm font-medium text-gray-800">{{ $item->hipotesis->nama ?? '—' }}</p>
                            @endif
                            <span class="text-xs text-gray-400">Self Diagnostik</span>
                        </div>
                        <div class="col-span-2">
                            @if($item->status === 'selesai' && $item->nilai_bayes_pakar)
                                <p class="text-sm font-bold text-gray-900">{{ $item->nilai_bayes_pakar }}%</p>
